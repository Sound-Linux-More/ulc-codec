...
void Fourier_DCT4(float *Buf, float *Tmp, int N)
{
    ...
    //!  u = R_n.x
    {
        ...
        {
            SrcHi -= FOURIER_VSTRIDE;
            b = FOURIER_VREVERSE(FOURIER_VLOAD(SrcHi));
            a = FOURIER_VLOAD(SrcLo);
            SrcLo += FOURIER_VSTRIDE;
            t1 = FOURIER_VMUL(s, a);
            t0 = FOURIER_VMUL(c, a);
            ...
            DstLo += FOURIER_VSTRIDE;
            FOURIER_VSTORE(DstHi, t1);
            DstHi += FOURIER_VSTRIDE;
            t0 = c;
            t1 = s;
    ...
    //!  y = (P_n)^T.w
    {
        ...
        *Dst++ = *TmpLo++;
#if FOURIER_VSTRIDE > 1
        {
            ...
            {
                TmpHi -= FOURIER_VSTRIDE;
                b = FOURIER_VREVERSE(FOURIER_VLOAD(TmpHi));
                a = FOURIER_VLOADU(TmpLo);
                TmpLo += FOURIER_VSTRIDE;
                t0 = FOURIER_VADD(a, b);
                t1 = FOURIER_VSUB(a, b);
                FOURIER_VINTERLEAVE(t0, t1, &a, &b);
                FOURIER_VSTOREU(Dst, a);
                Dst += FOURIER_VSTRIDE;
                FOURIER_VSTOREU(Dst, b);
                Dst += FOURIER_VSTRIDE;
            }
        }
#else
